...
<head>
  ...
  <title>Stats Pro – <?= h(app_config('brand.name', 'Pixels Studio')) ?></title>
    ...
    <section class="pro-grid">
      <header class="pro-header">
        <div>
          ...
          <h1 class="title">Stats Pro</h1>
          <p class="subtitle">Vista experimental para benchmark, rendimiento por operadores, tráfico por hora y salud comercial del embudo.</p>
        </div>
        <div class="pro-actions app-nav-actions">
          <?php nav_render_view_button('stats_advanced'); ?>
          <?php nav_render_account_switch($pdo, $accountOptions, $filterAccountId, 'stats_advanced.php', $selectedAccountParamsAll, $selectedAccountParamsAccount); ?>
          <?php nav_render_user_menu(true); ?>
        </div>
      </header>

      <form class="filters-card" method="get" action="<?= h(account_url('stats_advanced.php')) ?>">
      ...
      <section class="benchmark-grid" aria-label="Benchmark del periodo">
        <?php
          $metricCards = [
